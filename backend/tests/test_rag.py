...
def test_prompt_builder():
    ...
    with patch.object(
        service, "_load_template", return_value="Hello {{user}}, answer: {{answer}}"
    ):
...
def test_reranker_heuristics():
    ...
    chunk = DocumentChunk(
        id=uuid.uuid4(),
        document_id=uuid.uuid4(),
        content="TCP congestion control advantages",
        page_number=1,
        chunk_index=0,
        embedding=[],
        created_at=datetime.utcnow(),
...
def test_confidence_calculations():
    ...
    high_conf = service.calculate_confidence(
        [(chunk, 0.85), (chunk, 0.85), (chunk, 0.85)]
    )
...
def test_chat_service_sse_stream_generation(mock_generative_model):
    ...
    assert (
        len(sse_events) == 3
    )  # Yield 1: "Generated ", Yield 2: "response", Yield 3: Metadata citations
