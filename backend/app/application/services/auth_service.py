...
class AuthService:
    ...
    """
    def __init__(self, db: SqlSession, user_repo: IUserRepository, password_service: PasswordService, token_service: TokenService, session_service: SessionService, audit_service: AuditService):
        self.db = db
    ...
    def register_user(self, email: str, password: str, full_name: Optional[str], ip_address: str, device: str) -> User:
        """
        ...
        if existing_user:
            self.audit_service.log_action(None, "REGISTER", ip_address, device, "FAILED_EMAIL_TAKEN")
        ...
        self.audit_service.log_action(user_id, "REGISTER", ip_address, device, "SUCCESS")
        
        # Mocking email delivery (Log details in development console)
        logger.info(f"[SECURITY] Email Verification link generated for {email}: /verify-email?token={verification_token}")
    ...
    def login_user(
        ...
        if not user:
            self.audit_service.log_action(None, "LOGIN_FAILED", ip_address, user_agent_str, "USER_NOT_FOUND")
            raise InvalidCredentials("Invalid email or password.")

        # Check Account Lockout State
        if user.locked_until and user.locked_until > datetime.utcnow():
            self.audit_service.log_action(user.id, "ACCOUNT_LOCKED", ip_address, user_agent_str, "BLOCKED")
            locked_duration_min = int((user.locked_until - datetime.utcnow()).total_seconds() / 60)
            raise AccountLocked(f"Account locked due to consecutive failures. Try again in {locked_duration_min} minutes.")

        # Verify Password credentials
        is_valid_pw = self.password_service.verify_password(password, user.hashed_password)
        if not is_valid_pw:
            ...
            if user.failed_login_attempts >= 5:
                ...
                self.audit_service.log_action(user.id, "ACCOUNT_LOCKED", ip_address, user_agent_str, "LOCKED")
                logger.warning(f"[SECURITY] Account locked for email: {email} due to successive failures.")
            
            self.user_repo.update(user)
            self.db.commit()
            
            self.audit_service.log_action(user.id, "LOGIN_FAILED", ip_address, user_agent_str, "BAD_PASSWORD")
            raise InvalidCredentials("Invalid email or password.")
        ...
        access_token = self.token_service.create_access_token(user.id, user.role.value, token_family)
        raw_refresh_token, token_entity = self.token_service.create_refresh_token(user.id, token_family)
        ...
        self.audit_service.log_action(user.id, "LOGIN_SUCCESS", ip_address, user_agent_str, "SUCCESS")
        return access_token, raw_refresh_token, user

    def logout_user(self, user_id: uuid.UUID, token_family: uuid.UUID, ip_address: str, device: str) -> None:
        """
    ...
    def logout_all_sessions(self, user_id: uuid.UUID, ip_address: str, device: str) -> None:
        """
        ...
        self.audit_service.log_action(user_id, "LOGOUT_ALL", ip_address, device, "SUCCESS")

    def change_password(self, user_id: uuid.UUID, old_password: str, new_password: str, ip_address: str, device: str) -> None:
        """
        ...
        if not self.password_service.verify_password(old_password, user.hashed_password):
            self.audit_service.log_action(user_id, "PASSWORD_CHANGED", ip_address, device, "FAILED_BAD_OLD_PASSWORD")
        ...
        self.audit_service.log_action(user_id, "PASSWORD_CHANGED", ip_address, device, "SUCCESS")
    ...
    def forgot_password(self, email: str, ip_address: str, device: str) -> None:
        ...
        if not user:
            self.audit_service.log_action(None, "PASSWORD_RESET_REQUESTED", ip_address, device, "SUCCESS")
            logger.info(f"[SECURITY] Forgot password requested for non-existent email: {email}")
            return
        ...
        self.audit_service.log_action(user.id, "PASSWORD_RESET_REQUESTED", ip_address, device, "SUCCESS")
        logger.info(f"[SECURITY] Password reset link generated for {email}: /reset-password?token={reset_token}")

    def reset_password(self, token: str, new_password: str, ip_address: str, device: str) -> None:
        """
        ...
        self.audit_service.log_action(user.id, "PASSWORD_RESET", ip_address, device, "SUCCESS")
    ...
    def verify_email(self, token: str, ip_address: str, device: str) -> None:
        ...
        self.audit_service.log_action(user.id, "EMAIL_VERIFICATION", ip_address, device, "SUCCESS")
