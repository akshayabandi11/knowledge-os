...
class ChatService:
    ...
    def stream_chat(
        ...
        conversation_history_str = self.memory_service.get_history_as_string(conversation_id, max_messages=8)
        
        # 2. Rewrite query (resolving pronouns)
        rewritten_query = self.query_rewrite_service.rewrite_query(user_query, conversation_history_str)
        ...
        formatted_prompt = self.prompt_builder_service.build_prompt("chat.md", placeholders)
        ...
        citations = self.citation_service.extract_citations(full_response_text, pure_chunks_list)
        confidence = self.confidence_service.calculate_confidence(retrieved_chunks)
        
        # 8. Save assistant response to history
        self.memory_service.add_message(conversation_id, "assistant", full_response_text, citations)
        ...
        estimated_cost = self._estimate_cost(preferred_model, prompt_tokens, completion_tokens)
