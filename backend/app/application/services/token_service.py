...
from app.core.exceptions import TokenExpired, TokenRevoked, TokenReuseDetected, AuthenticationError
...
class TokenService:
    ...
    def create_access_token(self, user_id: uuid.UUID, role: str, token_family: uuid.UUID) -> str:
        """
    ...
    def create_refresh_token(self, user_id: uuid.UUID, token_family: uuid.UUID) -> tuple[str, RefreshToken]:
        """
    ...
    def rotate_refresh_token(self, raw_refresh_token: str) -> tuple[str, str]:
        ...
        if not token_record or token_record.revoked:
            ...
            logger_message = f"Refresh token reuse detected! Revoking token family: {token_family}"
            # Logger import deferred to avoid circular references
            from app.core.logging import logger
            logger.warning(logger_message)
            raise TokenReuseDetected("Token reuse detected. All sessions associated with this key have been revoked.")
        ...
        new_raw_refresh, new_token_entity = self.create_refresh_token(user_id, token_family)
