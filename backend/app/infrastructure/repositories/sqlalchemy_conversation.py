...
class SQLAlchemyConversationRepository(IConversationRepository):
    ...
    def get_by_id(self, id: UUID) -> Optional[Conversation]:
        db_conv = self.db.query(ConversationModel).filter(ConversationModel.id == id).first()
        if not db_conv:
            return None
    ...
    def update(self, entity: Conversation) -> Conversation:
        db_conv = self.db.query(ConversationModel).filter(ConversationModel.id == entity.id).first()
        if not db_conv:
    ...
    def get_by_user_id(self, user_id: UUID) -> List[Conversation]:
        db_convs = self.db.query(ConversationModel).filter(ConversationModel.user_id == user_id).order_by(ConversationModel.created_at.desc()).all()
        return [Conversation.model_validate(c) for c in db_convs]

    def get_messages(self, conversation_id: UUID, limit: int = 50) -> List[Message]:
        db_msgs = self.db.query(MessageModel).filter(
    ...
    def get_by_id_and_user_id(self, conversation_id: UUID, user_id: UUID) -> Optional[Conversation]:
        db_conv = self.db.query(ConversationModel).filter(
