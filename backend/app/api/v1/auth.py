...
def logout_all(
    ...
    auth_service.logout_all_sessions(
        user_id=current_user.id, ip_address=ip_address, device=user_agent
    )
    _clear_refresh_cookie(response)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
def forgot_password(
    ...
    auth_service.forgot_password(
        email=payload.email, ip_address=ip_address, device=user_agent
    )
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
def verify_email(
    ...
    auth_service.verify_email(
        token=payload.token, ip_address=ip_address, device=user_agent
    )
    return Response(status_code=status.HTTP_204_NO_CONTENT)
